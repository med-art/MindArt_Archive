...
function newGrid() {
  ...
  paint.image(bg,10,10,width,height);
  ...
  let fc3 = color(temp);
  fc3.setBlue(fc._getBlue()*.99);
  fc3.setRed(fc._getRed()*.99);
  fc3.setGreen(fc._getGreen()*.99);

  foreground.stroke(fc2);
  let sw = (gridLineSize * 3)-3;
  let sh = sw/6;
...
function brushIt(_x, _y, pX, pY) {
  ...
  if (brushSelected === 0) {
    ...
    paint.stroke(colorAlpha(colSelected, .5));
    ...
    paint.stroke(colorAlpha(colSelected, 0.5));
    for (i = 0; i < 30; i++) {
    ...
    for (i = 0; i < 30; i++) {
      paint.stroke(colorAlpha(colSelected, 0.4));
